...
public class MemCachedKMeansDriver extends AbstractJob {
	...
	private static final Logger log = LoggerFactory
			.getLogger(MemCachedKMeansDriver.class);
	...
	public static void run(Configuration conf, Path input, Path clustersIn,
		...
		Path clustersOut = buildClusters(conf, input, clustersIn, output,
				measure, maxIterations, delta, runSequential);
...
//	}
	...
	public static Path buildClusters(Configuration conf, Path input,
		...
		if (runSequential) {
			...
			return buildClustersMR(conf, input, output, measure,
					maxIterations, delta);
		}
	}

	private static Path buildClustersSeq(Configuration conf, Path input,
			Path clustersIn, Path output, DistanceMeasure measure,
			int maxIterations, String delta) throws IOException {
		...
		while (!converged && iteration <= maxIterations) {
			...
			for (VectorWritable value : new SequenceFileDirValueIterable<VectorWritable>(
			...
			Path clustersOut = new Path(output, AbstractCluster.CLUSTERS_DIR
					+ iteration);
			SequenceFile.Writer writer = new SequenceFile.Writer(fs, conf,
					new Path(clustersOut, "part-r-00000"), Text.class,
					Cluster.class);
			try {
						...
						log.debug(
								...
								new Object[] {
										...
										AbstractCluster.formatVector(
												cluster.getCenter(), null),
										cluster.getNumPoints(),
										AbstractCluster.formatVector(
												cluster.getRadius(), null),
										clustersOut.getName() });
					}
	...
	private static Path buildClustersMR(Configuration conf, Path input,
		...
		while (!converged && iteration <= maxIterations) {
			...
			Path clustersOut = new Path(output, AbstractCluster.CLUSTERS_DIR
					+ iteration);
			converged = runIteration(conf, input /* id list */, clustersOut,
	 ...
	 */
	private static boolean runIteration(Configuration conf, Path input,
			Path clustersOut, String measureClass,
			String convergenceDelta) throws IOException, InterruptedException,
...
//		job.setOutputFormatClass(SequenceFileOutputFormat.class);
		...
		if (!job.waitForCompletion(true)) {
			throw new InterruptedException(
					"K-Means Iteration failed processing ");
		}
	...
	public static void clusterData(Configuration conf, Path input,
			Path clustersIn, Path output, DistanceMeasure measure,
			String convergenceDelta, boolean runSequential) throws IOException,
			InterruptedException, ClassNotFoundException {
	...
	private static void clusterDataSeq(Configuration conf, Path input,
			Path clustersIn, Path output, DistanceMeasure measure)
			throws IOException {
		...
		for (FileStatus s : status) {
			SequenceFile.Writer writer = new SequenceFile.Writer(fs, conf,
					new Path(output, "part-m-" + part), IntWritable.class,
					WeightedVectorWritable.class);
			try {
				for (VectorWritable value : new SequenceFileValueIterable<VectorWritable>(
						s.getPath(), conf)) {
					clusterer.emitPointToNearestCluster(value.get(), clusters,
							writer);
				}
			} finally {
				Closeables.closeQuietly(writer);
	...
	private static void clusterDataMR(Configuration conf, Path input,
			Path clustersIn, Path output, DistanceMeasure measure,
			String convergenceDelta) throws IOException, InterruptedException,
			ClassNotFoundException {
